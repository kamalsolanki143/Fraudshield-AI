"""Fraud reporting and voting API router.

Handles submitting new community fraud cases, retrieving list and detailed reports,
and upvoting existing reports.
"""
...
from models.report import ReportCreate, ReportResponse, ReportSubmitResponse
from services.report_service import get_reports, submit_report
...
async def submit_new_report(
    ...
    Increments community points and awards achievement badges.
    """
    try:
...
@router.get(
    ...
    summary="Get paginated fraud reports list",
)
async def list_reports(
    ...
    """Retrieve all submitted fraud reports with optional filtering by type and category."""
...
async def get_report_detail(report_id: str):
    ...
    if "created_at" in report_doc and not isinstance(report_doc["created_at"], str):
...
async def upvote_report(
    ...
    """Increment the upvote count for a community report to signal verification."""
    db = get_db()
