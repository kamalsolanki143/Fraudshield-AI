"""
FraudShield AI — Fraud Report Pydantic Schemas
=================================================
Models for community fraud report submission, listing, and submission
...
class ReportCreate(BaseModel):
    ...
    entity_value: str = Field(
        ..., min_length=1, description="Value of the entity (UPI ID, phone, URL, message)"
    )
    scam_category: str = Field(
        ..., min_length=1, description="Scam category (e.g., refund_scam, fake_qr)"
    )
    description: str = Field(
        ..., min_length=5, description="Detailed description of the scam incident"
    )
...
class ReportSubmitResponse(BaseModel):
    """Response returned upon successfully submitting a report."""

    report_id: str = Field(..., description="UUID4 of the created report")
    points_awarded: int = Field(..., description="Guardian points awarded (e.g., 10)")
    badges_earned: List[str] = Field(
        default_factory=list, description="List of newly earned badges"
    )
