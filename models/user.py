"""User model schemas for input validation and API serialization.

Includes models for registration, login, updates, profiles, and token responses.
"""
...
class UserRegister(BaseModel):
    ...
    def validate_indian_phone(cls, v: str) -> str:
        ...
        clean_phone = re.sub(r"\s+", "", v)  # remove spaces
        # Match Indian mobile: starts with optional +91 or 91, followed by 6-9 and 9 digits
...
class UserProfile(BaseModel):
    """Schema representing a user profile returned in responses.
    ...
    fraud_iq_level: str = Field("novice", description="User's Fraud IQ level (novice/aware/vigilant)")
    ...
    updated_at: datetime = Field(..., description="Timestamp when updated")
...
class UserUpdate(BaseModel):
    ...
    fraud_iq_level: Optional[str] = Field(None, description="Updated Fraud IQ level (novice/aware/vigilant)")
